...
# COOL_GRACKLE_CHEMISTRY=2       # choose Grackle cooling chemistry: (0)=tabular, (1)=Atomic, (2)=(1)+H2+H2I+H2II, (3)=(2)+DI+DII+HD. Modules with dust and/or metal-line cooling require METALS also
...
MERGESPLIT_HARDCODE_MAX_MASS=(2.0e-8)   # manually set maximum mass for particle merge-split operations (in code units): useful for snapshot restarts and other special circumstances
MERGESPLIT_HARDCODE_MIN_MASS=(2.0e-9)   # manually set minimum mass for particle merge-split operations (in code units): useful for snapshot restarts and other special circumstances
