...
COM = coord_over[ind][np.argmax(density_over[ind])]
...
#     with open("spectrum_%03d.txt" % len, "w") as f:
#         np.savetxt(f, np.column_stack((k, E_spectrum)))

## Combine the two spectrum files into one
...
for i in spectrum_scale[1:]:
    ...
    old_offset = np.argmin(E_spectrum)
    new_offset_ind = np.where(k_new > k[old_offset])[0][0]
    # print(k_new[new_offset_ind], k[old_offset])

    amplitude_ratio = E_new[new_offset_ind] / E_spectrum[old_offset]

    k = np.concatenate( (k[:old_offset + 1], k_new[new_offset_ind:]) )
    E_spectrum = np.concatenate( (E_spectrum[:old_offset+1], E_new[new_offset_ind:] / amplitude_ratio)) 
...
# plt.show()
plt.savefig("k_spectrum.png")

## Save the combined spectrum to a file
with open("combined_spectrum.txt", "w") as f:
    np.savetxt(f, np.column_stack((k, E_spectrum)))